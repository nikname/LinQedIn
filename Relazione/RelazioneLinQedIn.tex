...
Ogni widget di tipo JobWidget e TitleWidget offre la possibilità di modifica e rimozione. La modifica avviene tramite la creazione di oggetti di tipo \textbf{EditJobWidget} ed \textbf{EditTitleWidget} e in caso la modifica avvenga all'ultima esperienza lavorativa o titolo di studio allora viene aggiornato l'header del profilo. La rimozione si preoccupa di fare invocare \texttt{delete} sul widget da rimuovere.

\subsubsection*{ConnectionsWidget}
Mostra gli utenti presenti nella rete dell'utente del quale si sta visualizzando il profilo. L'anteprima dei contatti viene creata con instanze di \textbf{UserPreviewWidget}. La disposizione è a griglia.
...
\subsubsection*{UserPreviewWidget}
